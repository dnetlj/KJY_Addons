...
class KjyPurchaseOrder(models.Model):
    ...
    #                                    default=lambda self: self.env['toproerp.common'].get_login_user()["id"])
    originator_hr_id = fields.Many2one('hr.employee', string=u'编制人', index=True,
    ...
    paid_amount = fields.Float(string=u'已付金额')
    ...
    warehousing = fields.Char(string=u'入库仓库')
    payment_type = fields.Char(string=u'付款类型')
